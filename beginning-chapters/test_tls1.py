...
def open_tls(context, address, server=False):
    ...
    if server:
        ...
        return context.wrap_sock(raw_client_sock, server_side=True)
    else:
        say('Address we want to talk to', address)
        raw_sock.connect(address)
        return context.wrap_sock(raw_sock)
...
def describe(ssl_sock, hostname, server=False, debug= False):
    ...
    if cert is None:
        ...
        names = [name for names in subject for (key, name) in names if key == 'DNS']
    ...
    say('Compression algorithmm in use', compression or 'none')
