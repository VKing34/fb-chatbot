...
B = 32  # number of bits precision in each random integer
...
def always_yes(zcontext, in_url, out_url):
    """Cordinates in the lower-left quadrant are inside the unit circle."""
...
def jugde(zcontext, in_url, pythagoras_url, out_url):
...
def pythagoras(zcontext, url):
    """Return the sum-of-squares of number sequeces."""
...
def start_thread(function, *args):
    ...
    thread.daemon = True # so you can easily Ctrrl-C the whole program
    thread.start()
...
def main(zcontext):
    ...
    start_thread(jugde, zcontext, pubsub, reqrep, pushpull)
